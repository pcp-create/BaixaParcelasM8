...
Antes de comparar CLIENTE com os complementos do M8, o sistema usa o texto após a primeira ocorrência de ` - `, removendo espaços nas extremidades. Sem esse separador, usa o texto completo. Exemplo: `PG.P/INTERNET - CD PROPAGANDA` vira `CD PROPAGANDA`. Separadores posteriores permanecem no nome; o valor exibido no extrato não muda. Nas sugestões, continua sendo exigida toda a descrição restante, na mesma ordem.

A identificação segue a mesma ordem na conciliação e nas sugestões: primeira palavra do cliente sem prefixo bancário em `pessoaNome` (da consulta ou da parcela); caso não corresponda, descrição completa sem prefixo no complemento do título; por último, descrição completa no complemento da própria parcela. São comparadas palavras inteiras com normalização de acentos, pontuação e caixa. Não se usa a antiga busca por qualquer palavra do complemento nem o prefixo de `fornecedorNome`. O complemento do título só participa quando fornecido pela API, sem chamada adicional à listagem geral.
