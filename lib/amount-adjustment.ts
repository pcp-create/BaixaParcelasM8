...
export function amountForMatching(row: Pick<NormalizedCsvRow, "valor" | "valorJuros" | "valorDesconto">): number | null {
  ...
  const discount = row.valorDesconto ?? 0;
  if (!Number.isFinite(discount) || discount < 0 || (discount > 0 && difference > 0) ||
      Math.abs(discount * 100 - Math.round(discount * 100)) > 0.000001) return null;
  const cents = Math.round(row.valor * 100) - Math.round(difference * 100) + Math.round(discount * 100);
...
export function applyAdjustment(row: NormalizedCsvRow, difference: number | null): NormalizedCsvRow {
  ...
  return {
    ...row, valorJuros: difference, valorDesconto: 0, sugestoesValor: undefined, status: "aguardando",
